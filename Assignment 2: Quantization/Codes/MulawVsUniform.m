...
% Mu-law Companding Process
u = 255; % Change values for different Mu values
for i=1:length(xsampling)
    y_companding(i) = sign(xsampling(i))*log(1+u*abs(xsampling(i)))/log(1+u);
end
...
for i=1:length(y_quantized)
   x_quantized(i) = sign(y_quantized(i))*((1+u)^abs(y_quantized(i))-1)/u;
end
...
grid on;
grid minor;
legend('Mu law Quantized Signal','Uniform Quantized Signal');
% Setting Font Size, Font Name, Font weight and Background color
set(gca, 'FontName', 'Consolas','fontsize',12,'FontWeight','bold');
set(gcf,'color','w');
...
title('Spectrum Plot of Mu Law Quantized Signal');
xlabel('Frequency in Hz');
ylabel('Magnitude');
set(gca, 'FontName', 'Consolas','fontsize',12,'FontWeight','bold');
set(gcf,'color','w');
